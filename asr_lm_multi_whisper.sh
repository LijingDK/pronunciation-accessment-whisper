...
expdir=exp_multitask           # Directory to save experiments.
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.gop_whisper_adapter \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/utt2score,phone_score,text_float" \
		--train_data_path_and_name_and_type "${_asr_train_dir}/utt2_new_word_score,word_score,text_float" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/utt2_new_utt_score,utt_score,text_float" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/text,text,text" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/utt2score,phone_score,text_float" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/utt2_new_word_score,word_score,text_float" \
                --valid_data_path_and_name_and_type "${_asr_valid_dir}/utt2_new_utt_score,utt_score,text_float" \
                --train_shape_file "${_logdir}/train.JOB.scp" \
                --valid_shape_file "${_logdir}/valid.JOB.scp" \
                --utt2align "${_asr_train_dir}/text_id_align_all" \
                --word_len_align "${_asr_train_dir}/text_word_align_all" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
	    ...
	    _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2score,phone_score,text_float "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2_new_word_score,word_score,text_float "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2_new_utt_score,utt_score,text_float "
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            ${_cmd} --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
                ${python} -m espnet2.bin.gop_inference \
	    ...
	    out_dir=embedding
            ...
            for f in token token_int score text embedding.scp; do
                if [ -f "${_logdir}/output.1/${out_dir}/${f}" ]; then
                  for i in $(seq "${_nj}"); do
                      cat "${_logdir}/output.${i}/${out_dir}/${f}"
                  done | sort -k1 >"${_dir}/${f}"
                fi
            done
        done
