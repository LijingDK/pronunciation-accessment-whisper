    ...
    def validate_one_epoch(
        ...
        for (utt_id, batch) in iterator:
            ...
            if isinstance(retval, dict):
                stats = retval["stats"]
                weight = retval["weight"]
                batch_phone_pred = retval["phone_pred"]
                batch_phone_true = retval["phone_true"]
                batch_word_pred = retval["word_pred"]
                batch_word_true = retval["word_true"]
                batch_utt_pred = retval["utt_pred"]
                batch_utt_true = retval["utt_true"]
                '''
                logging.warning(f"********valid**********")
                logging.warning(f"batch_phone_pred:{batch_phone_pred.shape} {batch_phone_pred}")
                logging.warning(f"batch_phone_true:{batch_phone_true.shape} {batch_phone_true}")
                logging.warning(f"batch_word_pred:{batch_word_pred.shape} {batch_word_pred}")
                logging.warning(f"batch_word_true:{batch_word_true.shape} {batch_word_true}")
                logging.warning(f"batch_utt_pred:{batch_utt_pred.shape} {batch_utt_pred}")
                logging.warning(f"batch_utt_true:{batch_utt_true.shape} {batch_utt_true}")
                '''

                if uu == 0:
                    total_phone_pred = batch_phone_pred
                    total_phone_true = batch_phone_true
                    total_word_pred = batch_word_pred
                    total_word_true = batch_word_true
                    total_utt_pred = batch_utt_pred
                    total_utt_true = batch_utt_true
                    uu = 1
                else:
                    total_phone_pred = np.hstack((total_phone_pred, batch_phone_pred))
                    total_phone_true = np.hstack((total_phone_true, batch_phone_true))
                    total_word_pred = np.hstack((total_word_pred, batch_word_pred))
                    total_word_true = np.hstack((total_word_true, batch_word_true))
                    total_utt_pred = np.hstack((total_utt_pred, batch_utt_pred))
                    total_utt_true = np.hstack((total_utt_true, batch_utt_true))
            ...
            if distributed:
                iterator_stop.fill_(1)
                torch.distributed.all_reduce(iterator_stop, ReduceOp.SUM)
        phone_mse = round(np.mean((total_phone_true - total_phone_pred) ** 2),3)
        word_mse = round(np.mean((total_word_true - total_word_pred) ** 2),3)
        utt_mse = round(np.mean((total_utt_true - total_utt_pred) ** 2),3)
        phone_pcc = round(np.corrcoef(total_phone_pred, total_phone_true)[0,1],3)
        word_pcc = round(np.corrcoef(total_word_pred, total_word_true)[0,1],3)
        utt_pcc = round(np.corrcoef(total_utt_pred, total_utt_true)[0,1],3)
        '''
        logging.warning(f"********valid**********")
        logging.warning(f"total_phone_pred:{total_phone_pred.shape} {total_phone_pred}")
        logging.warning(f"total_phone_true:{total_phone_true.shape} {total_phone_true}")
        logging.warning(f"total_word_pred:{total_word_pred.shape} {total_word_pred}")
        logging.warning(f"total_word_true:{total_word_true.shape} {total_word_true}")
        logging.warning(f"total_utt_pred:{total_utt_pred.shape} {total_utt_pred}")
        logging.warning(f"total_utt_true:{total_utt_true.shape} {total_utt_true}")
        '''
